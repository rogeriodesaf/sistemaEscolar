...
<body>
    ...
    <div id="box">
        <h1>Abaixo está mostrando todos os alunos do(a)
            <strong><?php echo $curso = base64_decode($_GET['curso']); ?></strong> <br>
            Data de Hoje <strong><?php date_default_timezone_set('America/Sao_Paulo');
    ...
    while ($res_1 = mysqli_fetch_assoc($resultado)) {
        ...
        <form name="button" method="post" enctype="multipart/form-data" action="">
            <table width="955" border="0">
                ...
                <tr>
                    <td><?php echo $res_1['code']; ?><input type="hidden" name="code_aluno"
                            value="<?php echo $res_1['code']; ?>" /></td>
                    <td><?php echo $res_1['nome']; ?><input type="hidden" name="nome"
            ...
            if ($presensa == '') {
                ...
                if (mysqli_num_rows($resultado3) == '' && $presensa == 'SIM') {
                    ...
                    if (mysqli_num_rows($resultado3) >= 1 && $presensa == 'NÃO' || $presensa == 'JUSTIFICADA') {
                        ?>
                    <td colspan="3">
                        <h3><strong>Este aluno entrou na instituição hoje, tem certeza que ele não está na sala de
                                aula?</strong></h3>
                        <a
                            href="fazer_chamada.php?curso=<?php echo $_GET['curso']; ?>&dis=<?php echo $_GET['dis']; ?>&confirmar_falta=sim&code_aluno=<?php echo $code_aluno; ?>&tipo=<?php echo $_POST['presensa']; ?>">CONFIRMAR
                            FALTA</a> | <a href="">CANCELAR</a>
                    </td>
                    <?php
} else {
